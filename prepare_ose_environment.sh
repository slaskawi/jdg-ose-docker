...
./oc project jdg
./oc delete all --all
./oc create -n jdg -f jboss-datagrid-image-stream.json
...
echo "Create JDG cluster: ./oc process datagrid65-basic -v APPLICATION_NAME=jdg | oc create -f -"
echo "Redeploy JDG cluster: ./oc deploy jdg --latest -n jdg"
